...
public class GameManager : MonoBehaviour {
	...
	private PlayerController localPlayerController2;
	...
	void FixedUpdate() {
		if (smartFox != null) {
			...
			*/
			if (localPlayer != null && localPlayer.GetComponent<VehicleControlPhysics>().MovementDirty)
			{
				Debug.Log("Updating Postion");
				List<UserVariable> userVariables = new List<UserVariable>();
				userVariables.Add(new SFSUserVariable("x", (double)localPlayer.rigidbody.position.x));
				userVariables.Add(new SFSUserVariable("y", (double)localPlayer.rigidbody.position.y));
				userVariables.Add(new SFSUserVariable("z", (double)localPlayer.rigidbody.position.z));
				smartFox.Send(new SetUserVariablesRequest(userVariables));
			}
	...
	public void OnUserVariableUpdate(BaseEvent evt) {
		...
		if (!remotePlayers.ContainsKey(user)) 
		{
			...
			Vector3 pos = new Vector3(58, 2, 52);
		...
		{
			Debug.Log("Remote player Change position");
	...
	private void SpawnLocalPlayer(int numModel, int numMaterial) {
		...
		localPlayer.AddComponent<VehicleControlPhysics>();
		localPlayer.AddComponent<igf_Movement_Player>();
		
		localPlayer.AddComponent<Rigidbody>();
		//localPlayerController2 = localPlayer.GetComponent<VehicleControlPhysics>();
		Camera.mainCamera.GetComponent<CarCamera>().target = localPlayer.transform;
